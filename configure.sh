#!/bin/bash
echo "Configuring Arithmetic Type..."

#./tools/cleanup.sh

export CMAKE_FLAGS="-DCMAKE_C_COMPILER=/usr/local/bin/clang -DCMAKE_CXX_COMPILER=/usr/local/bin/clang++ \
-DLIBCXX_INCLUDE_DIR=\"${LIBCXX_INCLUDE}\" -DLIBCXX_LIBRARY=\"${LIBCXX_LIB}\" \
-DBOOST_DIRS=\"/usr/local/include/boost\""
